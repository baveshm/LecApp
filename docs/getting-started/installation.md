...
This comprehensive guide covers deploying LecApp for production use, including detailed configuration options, performance tuning, and deployment best practices. While the Quick Start guide gets you running quickly, this guide provides everything you need for a robust production deployment.

## Understanding LecApp's Architecture

Before diving into installation, it's helpful to understand how LecApp works. The application integrates with external APIs for two main purposes: transcription services that convert your audio to text, and text generation services that power features like summaries, titles, and interactive chat. LecApp is designed to be flexible, supporting both cloud-based services like OpenAI and self-hosted solutions running on your own infrastructure.

LecApp uses specific API endpoint formats for these integrations. For transcription, it supports the standard OpenAI Whisper API format using the `/audio/transcriptions` endpoint, which is implemented by OpenAI, OpenRouter, and many self-hosted solutions. Alternatively, for advanced features like speaker diarization, LecApp can connect to ASR webservices that provide an `/asr` endpoint. **Note: Using the ASR endpoint option requires running an additional Docker container** (`onerahmet/openai-whisper-asr-webservice`) alongside LecApp - full setup instructions are provided in the [Running ASR Service for Speaker Diarization](#running-asr-service-for-speaker-diarization) section below. For text generation, LecApp uses the OpenAI Chat Completions API format with the `/chat/completions` endpoint, which is widely supported across different AI providers.
...
You have two main options for deploying LecApp. The first and recommended approach is using the pre-built Docker image from Docker Hub, which requires no source code and gets you running quickly. The second option is building from source, which is useful if you need to modify the code or prefer to build your own images. Both methods use Docker Compose for orchestration and management.

## Standard Installation Using Pre-Built Image

### Step 1: Create Installation Directory

Choose an appropriate location for your LecApp installation. This directory will contain your configuration files and data volumes. For production deployments, using a dedicated directory like `/opt/lecapp` or `/srv/lecapp` is recommended as it provides a clear separation from user home directories and follows Linux filesystem hierarchy standards.
...
If you're just testing or running LecApp for personal use, you can create the directory in your home folder instead. The location isn't critical, but keeping everything organized in one place makes management easier.
...
The restart policy `unless-stopped` ensures LecApp automatically starts after system reboots unless you've explicitly stopped it. The volumes mount local directories for persistent storage of uploads and database files.

### Step 3: Environment Configuration

The environment configuration is where you tell LecApp which AI services to use and how to connect to them. Download the appropriate environment template based on your transcription service choice. This template contains all the configuration variables with helpful comments explaining each setting.
...
If you want speaker diarization to identify who's speaking in your recordings, you'll need to use an ASR webservice endpoint. **This requires running an additional Docker container** (`onerahmet/openai-whisper-asr-webservice`) alongside LecApp, but provides powerful features for meeting transcriptions and multi-speaker recordings.
...
The ASR configuration enables the custom endpoint and tells LecApp where to find it:
...
The ASR_BASE_URL depends on your deployment architecture. If you're running the ASR service in the same Docker Compose stack as LecApp, use the service name from your docker-compose.yml file, like `http://whisper-asr:9000`. This uses Docker's internal networking for communication. If the ASR service is running elsewhere, use its full URL with the appropriate IP address or domain name.
...
One of LecApp's conveniences is automatic admin account creation. Instead of going through a registration process, you define the admin credentials in your environment file, and LecApp creates the account automatically on first startup. This ensures you can log in immediately without any additional setup steps:
...
Setting `ALLOW_REGISTRATION=false` means only the admin can create new user accounts, which is recommended for private installations where you want to control access. If you're running LecApp for a team or family, this prevents random people from creating accounts. The timezone setting affects how dates and times are displayed throughout the interface, so set it to your local timezone for convenience. The log level controls how much information LecApp writes to its logs. Use `INFO` during initial setup and testing to see what's happening, then switch to `ERROR` for production to reduce log volume and improve performance.

### Step 5: Configure Advanced Features

#### Large File Handling

One of LecApp's most useful features is automatic handling of large audio files. Many transcription APIs have file size limits, with OpenAI's 25MB limit being a common constraint. Rather than forcing you to manually split files, LecApp handles this automatically through intelligent chunking:
...
Inquire Mode transforms LecApp from a simple transcription tool into a knowledge base of all your recordings. When enabled, you can search across all your transcriptions using natural language questions:
...
LecApp needs local directories to store your data persistently. These directories are mounted as Docker volumes, ensuring your recordings and database survive container updates and restarts:
...
### Step 7: Launch LecApp

With everything configured, you're ready to start LecApp. The `-d` flag runs the container in detached mode, meaning it continues running in the background:

```bash
docker compose up -d
```

The first time you run this command, Docker will download the LecApp image from Docker Hub. This image is approximately 3GB and contains all the dependencies needed to run LecApp, including FFmpeg for audio processing and various Python libraries. The download time depends on your internet connection speed.
...
Open your web browser and navigate to `http://your-server:8899`, replacing `your-server` with your actual server address or `localhost` if you're running locally. You should see the LecApp login page with its distinctive gradient design.
...
If you need speaker diarization to identify different speakers in your recordings, you'll need to run an ASR service alongside LecApp. This involves deploying an additional Docker container (`onerahmet/openai-whisper-asr-webservice`) that provides the ASR endpoint. The recommended approach is running both containers in the same Docker Compose stack for simplified networking and management.
...
If you prefer to manage the services independently or are adding the ASR service to an existing LecApp installation, you can run them in separate Docker Compose files. This approach gives you more flexibility and works whether the services are on the same machine or different machines.
...
```bash
docker network create lecapp-network
```
...
Update your LecApp `docker-compose.yml` to use the shared network:
...
On the LecApp server, use the standard `docker-compose.yml`:
...
In your LecApp `.env` file, use the ASR server's IP address or hostname:
```bash
# Using IP address
...
The WebSocket configuration is important for real-time features in LecApp. The timeout settings ensure large file uploads don't get interrupted. You can obtain free SSL certificates from Let's Encrypt using Certbot, making HTTPS accessible for everyone.

### Backup Strategy

Regular backups are essential for production deployments. Your LecApp data consists of three critical components that need backing up: the SQLite database in the `instance` directory, the audio files and transcriptions in the `uploads` directory, and your configuration in the `.env` file.

Create a backup script that captures all three components:

```bash
#!/bin/bash
BACKUP_DIR="/backup/lecapp"
...
# Create the backup
tar czf "$BACKUP_DIR/lecapp_backup_$DATE.tar.gz" \
    /opt/lecapp/instance \
    /opt/lecapp/uploads \
    /opt/lecapp/.env

# Optional: Keep only last 30 days of backups
find "$BACKUP_DIR" -name "lecapp_backup_*.tar.gz" -mtime +30 -delete

echo "Backup completed: lecapp_backup_$DATE.tar.gz"
```

Make the script executable and schedule it with cron for automated daily backups:

```bash
chmod +x /opt/lecapp/backup.sh
crontab -e
# Add this line for daily backups at 2 AM:
0 2 * * * /opt/lecapp/backup.sh
```
...
```bash
#!/bin/bash
USAGE=$(df /opt/lecapp | tail -1 | awk '{print $5}' | sed 's/%//')
if [ $USAGE -gt 80 ]; then
    echo "Warning: LecApp disk usage is at ${USAGE}%" | mail -s "LecApp Disk Alert" admin@example.com
fi
```
...
Restrict network access using firewall rules. If LecApp is only used internally, limit access to your organization's IP ranges:
...
```nginx
limit_req_zone $binary_remote_addr zone=lecapp:10m rate=10r/s;
limit_req zone=lecapp burst=20;
```
...
Keeping LecApp updated ensures you have the latest features and security patches. The update process is straightforward but should be done carefully to avoid data loss.

First, always create a backup before updating:

```bash
# Create a backup
tar czf lecapp_backup_before_update.tar.gz uploads/ instance/ .env
...
```yaml
image: learnedmachine/lecapp:v1.2.3  # Replace with your previous version
```
...
If the port is in use, either stop the conflicting service or change LecApp's port in docker-compose.yml.
...
If you need to modify LecApp's code or prefer building your own images, you can build from source. This requires cloning the repository and using Docker's build capability.

First, clone the repository and navigate to it:

```bash
git clone https://github.com/baveshm/LecApp.git
cd lecapp
```
...
services:
  app:
    ...
    image: lecapp:custom  # Tag for your custom build
    container_name: lecapp
    restart: unless-stopped
    ports:
      - "8899:8899"
...
This ensures LecApp has enough resources while preventing it from consuming everything on shared servers.
...
Next: [User Guide](../user-guide/index.md) to learn how to use all of LecApp's features